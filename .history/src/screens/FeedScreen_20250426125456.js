    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.card, borderBottomColor: theme.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.text }]}>
          {route.params?.categoryName || 'News'}
        </Text>
      ...
      {/* News List */}
      <NewsListContent 
        category={route.params?.categoryId || 'top'} 
        theme={theme} 
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity 
      ...
      onPress={() => navigation.navigate('NewsDetail', { 
        article: item,
        isDark: isDark // Pass the current theme mode to detail screen
      })}
      activeOpacity={0.8}
    >
      {item.image_url && (
...
const CustomDrawerContent = (props) => {
  const { state, navigation, isDark, setIsDark } = props;
  const theme = isDark ? themes.dark : themes.light;

  return (
    <View style={[styles.drawerContainer, { backgroundColor: theme.card }]}>
      <View style={styles.drawerHeader}>
        <Text style={[styles.drawerTitle, { color: theme.primary }]}>News Categories</Text>
      </View>
      ...
      {categories.map((category) => (
        <TouchableOpacity
          ...
          style={[
            styles.drawerItem,
            state.routeNames[state.index] === category.id && { 
        ...
        >
          <Text style={[
            ...
            { 
              color: state.routeNames[state.index] === category.id 
                ? theme.primary 
                : theme.text,
            }
          ]}>
